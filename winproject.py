...
class WDMDriverAnalysis(angr.Project):
	...
	def recovery_ioctl_interface(self):
		...
		if self.allowed_call_mode:
			self.use_allowed_call_mode(state, [arg_iostacklocation, 'IoControlCode', 'InputBuffer', 'CurrentStackLocation'])
		...
		for ioctl_code, state in switch_states.items():
			state.inspect.b('mem_read',
				condition=lambda st: 'SystemBuffer' in str(st.inspect.mem_read_address),
			...
			simgr = self.project.factory.simgr(state)
			simgr.run(until=lambda x: len(self.ioctl_constraints))
